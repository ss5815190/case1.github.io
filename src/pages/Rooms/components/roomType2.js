...
const RoomType2 =({RoomCard,Room3})=>{
	const [currentId,setCurrentId]=useState(0)
	const next=()=>{
		setCurrentId((currentId)=>Room3.length-1>currentId?currentId+1:0);
	}
	const prev=()=>{
		setCurrentId((currentId)=>currentId>0?currentId-1:Room3.length-1);
	}
	useEffect(()=>{
		setCurrentId(0);
	...
	return(			
			<div className="roomCard-grid" >
					...
					<div className="roomCard-detail">
						<div className="roomCard-detail-title">{RoomCard[2].roomName}</div>
					<div className="roomCard-detail-price">{RoomCard[2].price}</div>
					</div>
